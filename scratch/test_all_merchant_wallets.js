async function testAllMerchantWallets() {
    ...
    try {
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          currency: "GHS",
          asset: comb.asset,
          network: comb.network,
          description: "Test Purchase",
          forceRateRefresh: true
        })
      });
      const text = await res.text();
      console.log(`Asset "${comb.asset}" (${comb.network}) => Status ${res.status}:`, text);
    } catch (e) {
      console.log(`Error ${comb.asset}:`, e.message);
    }
  }
}

testAllMerchantWallets().catch(console.error);
